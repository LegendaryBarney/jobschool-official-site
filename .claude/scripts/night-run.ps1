...
#   code，而是讀 CLAUDE.md / PROGRESS，再透過 Workflow 工具執行名為
#   `night-orchestrate` 的編排腳本（.claude/workflows/night-orchestrate.mjs），
#   由它 fan-out 多個 subagent 完成規劃→切割→平行實作測試→序列收斂→匯總。
#
...
3. 確保在 feature branch 上工作：先 git rev-parse --abbrev-ref HEAD 看目前分支；
   若在 main/master，立刻 git switch -c feat/night-<日期>（日期用系統或 git log 取得）。
   絕不在 main 上動工或 push。
4. 用 Workflow 工具執行名為 `night-orchestrate` 的編排腳本，傳入 args：
     {
   ...
   （branch / night 由 KICKOFF front-matter 提供，Workflow 內的 PLANNER 會解析；
    你不必另外傳。）這個 Workflow 會自己 fan-out PLANNER / SPLITTER / 多個
    IMPLEMENT+TEST / INTEGRATOR / REPORTER subagent，完成規劃→切割→平行實作測試
    →序列收斂→匯總，並在 token 用滿 $BudgetCap 或隊列清空時收尾。
5. Workflow 回傳結構化總結後：確認 REPORTER 已把當晚狀態寫進 PROGRESS.md
   （逐項狀態表 / 卡在哪 / 下一步 / 待決策 / 預算）。若 PROGRESS 尚未更新，
   讓一個 subagent 依 Workflow 回傳補寫。最後簡短回報本晚摘要。
